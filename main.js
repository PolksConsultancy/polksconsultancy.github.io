...
var APP = {
    ...
    init: async function() {
        ...
        APP.extensionFirebaseConfig = APP.extensionAPI+"firebaseConfig";
    ...
    inboxChatSetup: async function() {
        ...
        if(APP.module && APP.recordId) {
            ...
            $("#recordDetailsSelectOption").remove();
        ...
        if((data && data.data && data.data.length) || (latestRecordsList && latestRecordsList.length)) {
            ...
            if(loadedMessages.length > 0) {
                ...
                loadedMessages.forEach(async function(message) {
                    await APP.addMessage(message[APP.extensionFieldMsgId], contactId, "loaded", !isInitLoad);
                });
            }
        }
    },
    ...
    histroyAction: async function(contactId, messageId) {
        ...
        if(histroyRecordMap && histroyRecordMap.id) {
            ...
            let searchRecord = await ZOHO.CRM.API.searchRecord({Entity:APP.extensionHistory, Type:"criteria",Query:`(${APP.extensionFieldMsgId}:equals:${decodeURIComponent(messageId).replaceAll("_", ".").replaceAll("-", "=")})`});
            if(searchRecord.data) {
                histroyRecordMap.id = searchRecord.data[0].id;
                APP.contacts[contactId].messages[messageId].id = histroyRecordMap.id;
                await ZOHO.CRM.API.updateRecord({Entity: APP.extensionHistory, APIData: histroyRecordMap, Trigger:["workflow"]}).then(function(data){
                    // console.log(data);
                });
            }
                ...
                await ZOHO.CRM.API.insertRecord({Entity: APP.extensionHistory, APIData: histroyRecordMap, Trigger:["workflow"]}).then(function(data){
                    // console.log(data);
                    APP.contacts[contactId].messages[messageId].id = data.data[0].details.id;
                });
            }
        }
    },
    ...
    addMessage: async function(messageId, contactId, type, isLoadMore=false) {
        ...
        if(message[APP.extensionFieldEncodeMessage]) {
            ...
            let reactionFrom = message[APP.extensionFieldReactionFrom] ? `<div><span>${APP.svg.bottomArrow}</span><span>${decodeURIComponent(decodeURIComponent(message[APP.extensionFieldReactionFrom]))}</span></div>` : '';
            let reactionTo = message[APP.extensionFieldReactionTo] ? `<div><span>${APP.svg.topArrow}</span><span>${decodeURIComponent(decodeURIComponent(message[APP.extensionFieldReactionTo]))}</span></div>` : '';

            let rections = reactionFrom && reactionTo ? reactionFrom+reactionTo : reactionFrom ? reactionFrom : reactionTo ? reactionTo : '';
                                        ...
                                        <div class="${ messageDirection }">
                                            <div class="message-content-main">
                                                ...
                                                <div class="message-content-main-div">
                                                    ...
                                                    <span class="">${messageToReact}<div class="message-text-to-out"><div class="message-text-to-in"><span class="message-text-to"><svg viewBox="0 0 18 18" height="18" width="18" preserveAspectRatio="xMidYMid meet" class="" version="1.1" x="0px" y="0px" enable-background="new 0 0 18 18"><title>down-context</title><path fill="currentColor" d="M3.3,4.6L9,10.3l5.7-5.7l1.6,1.6L9,13.4L1.7,6.2L3.3,4.6z"></path></svg></span></div></div></span>
                                                </div>  
    ...
    firebaseSetup: async function() {
        APP.firebaseConfig = await APP.getOrgVariable(APP.extensionFirebaseConfig);
        if(APP.firebaseConfig && APP.firebaseConfig.databaseURL)
        {
    ...
    getOrgVariable: async function(variableAPI) {
        return await ZOHO.CRM.API.getOrgVariable(variableAPI).then(function(apiKeyData){
            if(apiKeyData && apiKeyData.Success && apiKeyData.Success.Content && apiKeyData.Success.Content != "0"){
                return APP.parseJsonObjectStr(apiKeyData.Success.Content);
            }
            else {
                return {};
            }
    ...
    realtimeListener: function() {
        APP.database.ref('incomingMessages').on('child_added', async (snapshot) => {
            ...
            let type = data.messages[0].type;
            let text = data.messages[0].text && data.messages[0].text.body ? data.messages[0].text.body : '';
            let name = data.contacts && data.contacts[0] && data.contacts[0].profile && data.contacts[0].profile.name ? data.contacts[0].profile.name : "";
            let timestamp = data.messages[0].timestamp;
            ...
            let contactId = from;
            let contact = APP.contacts[contactId] ? APP.contacts[contactId] : {};

            let contactName = "incoming from "+ contact.details && contact.details[APP.extensionFieldName] ? contact.details[APP.extensionFieldName] : name ? name : contactId;
            let owner = contact.details && contact.details[APP.extensionFieldOwner] && contact.details[APP.extensionFieldOwner].id ? contact.details[APP.extensionFieldOwner].id : '';
            let time = Number(timestamp) * 1000;          
            let encodeText = encodeURIComponent(encodeURIComponent(text));

            let contactDetails = contact.details ? contact.details : {};
            contactDetails[APP.extensionFieldName] = contactName;
            contactDetails[APP.extensionFieldOwner] = owner;
            contactDetails[APP.extensionFieldWhatsAppNumber] = contactId;
            if(Object.keys(contact).length > 0) {
                contact.details = APP.removeEmptysInObject(contactDetails);
            }
            else {
                await ZOHO.CRM.API.searchRecord({Entity: APP.extensionContacts, Type:"phone", Query: contactId, delay:false, sort_by: APP.extensionFieldActiveTime, sort_order: "desc"}).then( async function(data){
                    if(data && data.data) {
                        contactDetails[APP.extensionFieldId] = data.data[0].id;
                    }
                    else {
                        await ZOHO.CRM.API.searchRecord({Entity: "Contacts", Type:"phone",Query:contactId, delay:false}).then( async function(data){
                        if(!data || !data.data) {
                            await ZOHO.CRM.API.searchRecord({Entity: "Leads", Type:"phone",Query:contactId, delay:false}).then(async function(resp){
                                if(!resp || !resp.data) {
                                    let lastname = contactName;
                                    await ZOHO.CRM.API.insertRecord({Entity: "Leads",APIData:{Last_Name: lastname, Phone: contactId},Trigger:["workflow"]}).then(function(response){
                                        contactDetails[APP.extensionFieldName] = lastname;
                                        contactDetails[APP.extensionFieldModule] = "Lead";
                                        contactDetails[APP.extensionFieldLead] = response.data[0].details.id;
                                    });
                                }
                                else {
                                    contactDetails[APP.extensionFieldName] = resp.data[0].Full_Name;
                                    contactDetails[APP.extensionFieldModule] = "Lead";
                                    contactDetails[APP.extensionFieldLead] = resp.data[0].id;
                                }
                            });
                        }
                        else {
                            contactDetails[APP.extensionFieldName] = data.data[0].Full_Name;
                            contactDetails[APP.extensionFieldModule] = "Contact";
                            contactDetails[APP.extensionFieldContact] = data.data[0].id;
                        }
                    });
                    }
                });
                contact = {
                    id: contactId,
                    unread: 0,
                    details: APP.removeEmptysInObject(contactDetails),
                    notifications: {},
                    messages: {},                        
                    pageCompleted: false,
                    currentPage: 1
            ...
            let message = messages[message_id] ? messages[message_id] : {};

            if(Object.keys(message).length <= 0) {
                let searchRecord = await ZOHO.CRM.API.searchRecord({Entity:APP.extensionHistory, Type:"criteria",Query:`(${APP.extensionFieldMsgId}:equals:${message_id})`});
                if(searchRecord.data) {
                    message = searchRecord.data[0];
                }
            }
            
            message[APP.extensionFieldMsgId] = message_id;
            if(reactionFrom) {
                message[APP.extensionFieldReactionFrom] = encodeURIComponent(encodeURIComponent(reactionFrom));
            }
            else {
                if(replyMessageId) {
                ...
                message[APP.extensionFieldTimestamp] = time;
                contact.details[APP.extensionFieldActiveTime] = time;
            }
            
            messages[message_id] = APP.removeEmptysInObject(message);
            contact.messages = APP.removeEmptysInObject(messages);

            let contactNotifications = contact.notifications ? contact.notifications : {};
            contactNotifications[key] = message;
            contact.notifications = contactNotifications;

            contact.details[APP.extensionFieldLastMessage] = message;
            APP.contacts[contactId] = contact;
            ...
            else if(type == "text" && APP.currentContactId != contactId) {
        ...
        APP.database.ref('outgoingMessages').on('child_added', async (snapshot) => {
            let data = snapshot.val();
            let key  = snapshot.key;
            if(APP.realtimeDuplicateChaeckArr[key]) return;
            APP.realtimeDuplicateChaeckArr[key] = key;
            ...
            let name = data.contacts && data.contacts[0] && data.contacts[0].profile && data.contacts[0].profile.name ? data.contacts[0].profile.name : "";
            let timestamp = data.statuses[0].timestamp;
            let status = data.statuses[0].status;   

            let contactId = to;
            let contact = APP.contacts[contactId] ? APP.contacts[contactId] : {};

            let contactName = "Outgoing to "+ contact.details && contact.details[APP.extensionFieldName] ? contact.details[APP.extensionFieldName] : name ? name : contactId;
            let owner = contact.details && contact.details[APP.extensionFieldOwner] && contact.details[APP.extensionFieldOwner].id ? contact.details[APP.extensionFieldOwner].id : '';
            let time = Number(timestamp) * 1000;
            ...
            delete APP.contacts[contactId].notifications[key];
            setTimeout(() => {
                APP.database.ref('outgoingMessages/'+key).remove();
            }, 2000);
            if(!APP.filterModes["all"].contacts.includes(contactId)) {
                APP.filterModes["all"].contacts.push(contactId);
            }
    ...
    removeEmptysInObject: function(jsonObject) {
        // jsonObject = Object.fromEntries(Object.entries(jsonObject).filter(([_, v]) => v !== null && v !== NaN && v !== undefined && v !== ''));
        return Object.fromEntries(Object.entries(jsonObject).filter(([key, value]) => value));
    },
    parseJsonObjectStr: function(jsonObjectStr) {
    ...
    editPopupDiv: async function(e, contactId, module, recordId, fieldId) {
        ...
        e.target.closest(".field-row").insertAdjacentHTML("beforeend", popupHtmlContent);
    },
