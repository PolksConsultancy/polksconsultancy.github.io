...
var APP = {
    ...
    realtimeListener: function() {
        APP.database.ref('incomingMessages').on('child_added', async (snapshot) => {
            ...
            let time = contactId && APP.contacts[contactId] && APP.contacts[contactId].messages && APP.contacts[contactId].messages[message_id] && APP.contacts[contactId].messages[message_id][APP.extensionFieldTimestamp] ? new Date(APP.contacts[contactId].messages[message_id][APP.extensionFieldTimestamp]).getTime() : Number(data.messages[0].timestamp) * 1000;
